# Microsoft Visual C++ Redistributable
winget install -e --id Microsoft.VCRedist.2005.x86 # Microsoft Visual C++ 2005 Redistributable x86
winget install -e --id Microsoft.VCRedist.2005.x64 # Microsoft Visual C++ 2005 Redistributable x64

winget install -e --id Microsoft.VCRedist.2008.x86 # Microsoft Visual C++ 2008 Redistributable x86
winget install -e --id Microsoft.VCRedist.2008.x64 # Microsoft Visual C++ 2008 Redistributable x64

winget install -e --id Microsoft.VCRedist.2010.x86 # Microsoft Visual C++ 2010 Redistributable x86
winget install -e --id Microsoft.VCRedist.2010.x64 # Microsoft Visual C++ 2010 Redistributable x64

winget install -e --id Microsoft.VCRedist.2012.x86 # Microsoft Visual C++ 2012 Redistributable x86
winget install -e --id Microsoft.VCRedist.2012.x64 # Microsoft Visual C++ 2012 Redistributable x64

winget install -e --id Microsoft.VCRedist.2015+.x86 # Microsoft Visual C++ 2015-2022 Redistributable x86
winget install -e --id Microsoft.VCRedist.2015+.x64 # Microsoft Visual C++ 2015-2022 Redistributable x64

# Base apps
winget install -e --id Brave.Brave # Brave
winget install -e --id Discord.Discord # Discord
winget install -e --id Telegram.TelegramDesktop # Telegram
winget install -e --id 7zip.7zip # 7-zip
winget install -e --id ShareX.ShareX # ShareX
winget install -e --id VideoLAN.VLC # VLC

# Devs
winget install -e --id Git.Git # Git
winget install -e --id OpenJS.NodeJS # Node.JS
winget install -e --id Yarn.Yarn # Yarn
winget install -e --id Microsoft.VisualStudioCode # VSCode
winget install -e --id Docker.DockerDesktop # Docker Desktop
winget install -e --id MongoDB.Compass.Full # MongoDB Compass
winget install -e --id Insomnia.Insomnia # Insomnia

# Terminal 
winget install -e --id Microsoft.PowerShell #PowerShell 7+
winget install -e --id JanDeDobbeleer.OhMyPosh # OhMyPosh
winget install -e --id gerardog.gsudo # gsudo

# Games Launchers
winget install -e --id Valve.Steam #Steam
winget install -e --id EpicGames.EpicGamesLauncher # Epic Games Launcher
winget install -e --id Playnite.Playnite # Playnite

# For Gaming
winget install -e --id Soneliem.WAIUA # WAIUA

# Other
winget install -e --id Microsoft.PowerToys # PowerToys
winget install -e --id qBittorrent.qBittorrent # qBittorrent
winget install -e --id Spotify.Spotify --force # Spotify
winget install -e --id BellSoft.LibericaJDK.19.Full # JDK
winget install -e --id File-New-Project.EarTrumpet # EarTrumpet
winget install -e --id OpenVPNTechnologies.OpenVPN # OpenVPN
winget install -e --id Bitwarden.Bitwarden # BitWarden
winget install -e --id Obsidian.Obsidian # Obsidian
winget install -e --id Nvidia.GeForceExperience # GeForceExperience
