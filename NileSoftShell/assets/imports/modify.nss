...
remove(find="*GIT*")
remove(find="ArmouryCrate")
remove(find="*NVIDIA")
remove(find="terminals")
remove(find="*ShareX*")
modify(find="Run with priority" title="Запустити з пріоритетом")
remove(find="Take Ownership")
remove(find="Додати до вподобань")
remove(find="*Пуск*")
remove(find="*Блокнот*")
remove(find="*Powershell*")
remove(find="*Linux*")
modify(find="Open with Code" title="Відкрити в VSCode")
modify(find="Open with Code" title="Відкрити в VSCode")
remove(find="Sign and encrypt")
remove(find="*GPG*")